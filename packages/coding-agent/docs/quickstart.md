...
npm uninstall -g @schovest/pi-coding-agent

# pnpm
pnpm remove -g @schovest/pi-coding-agent

# Yarn
yarn global remove @schovest/pi-coding-agent

# Bun
bun uninstall -g @schovest/pi-coding-agent
```
...
Additional built-in read-only tools (`grep`, `find`, `ls`) are available through tool options. All `--tools` and `--exclude-tools` options support glob patterns (e.g., `--tools "read*"`). Pi runs in your current working directory and can modify files there. Use git or another checkpointing workflow if you want easy rollback.
...
Use `/model` or Ctrl+L to choose a model. Use Shift+Tab to cycle primary agents. Use Ctrl+P / Shift+Ctrl+P to cycle through scoped models.
