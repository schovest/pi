...
Fired before pi decides whether to trust a project with dynamic configs (`.pi` or, when `enableAgentsSkills` is `true`, `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
Fired when a session's metadata (e.g. display name) changes. Fired both when the current session is renamed (`/name`, `pi.setSessionName()`, or the picker's Ctrl+R) and when another session is renamed from the picker.

```typescript
pi.on("session_info_changed", async (event, ctx) => {
  // event.sessionFile - session file whose name changed (absent for non-persisted sessions)
  // event.name - current session name, or undefined when cleared
  console.log(`Session ${event.sessionFile} name changed to: ${event.name ?? "(cleared)"}`);
});
```
