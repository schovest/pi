import type { CredentialStore } from "@earendil-works/pi-ai/compat";
...
export async function createModelRegistry(credentials: CredentialStore, modelsPath?: string): Promise<ModelRegistry> {
	return wrap(
		await ModelRuntime.create({ credentials, modelsPath, allowModelNetwork: false, modelNetworkEnabled: false }),
	);
}
