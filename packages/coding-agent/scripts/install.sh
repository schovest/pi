...
# Install context-mode plugin (interactive)
echo ""
read -r -p "Install context-mode plugin? [Y/n] " INSTALL_CONTEXT_MODE
INSTALL_CONTEXT_MODE="${INSTALL_CONTEXT_MODE:-Y}"

if [[ "$INSTALL_CONTEXT_MODE" =~ ^[Yy]$ ]]; then
	...
	echo "Skipping context-mode plugin installation."
fi