	...
	try {
		...
		handle = await startUiServer({
			...
			onComplete: (reason: string) => {
				...
				if (currentHandle && state.uiServer === currentHandle) {
					const messages = currentHandle.getSessionMessages();
					const stream = currentHandle.getStreamSummary();
					const hasContent =
					...
					if (hasContent) {
						state.completedUiSessions.push({
							serverName: currentHandle.serverName,
							toolName: currentHandle.toolName,
