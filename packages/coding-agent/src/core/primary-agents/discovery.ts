...
import type { PrimaryAgentDefinition, PrimaryAgentDefinitionScope, DiscoverPrimaryAgentsOptions } from "./types.ts";
	...
	return {
		...
		model:
			typeof parsed.frontmatter.model === "string"
				...
				: undefined,
		thinking: isThinkingLevel(parsed.frontmatter.thinking)
			? parsed.frontmatter.thinking
			: undefined,
	};
}

function loadPrimaryAgentDir(
	path: string,
	scope: PrimaryAgentDefinitionScope,
): PrimaryAgentDefinition[] {
...
function mergePrimaryAgents(
	groups: PrimaryAgentDefinition[][],
): PrimaryAgentDefinition[] {
...
export function discoverPrimaryAgentsSync(
	options: DiscoverPrimaryAgentsOptions,
): PrimaryAgentDefinition[] {
	const builtIn = BUILT_IN_PRIMARY_AGENTS.map((agent) => ({ ...agent }));
	const user = loadPrimaryAgentDir(
		join(options.agentDir, "primary-agents"),
		"user",
	);
	const projectDir = findNearestProjectPrimaryAgentsDir(options.cwd);
	const project = projectDir
		? loadPrimaryAgentDir(projectDir, "project")
		: [];
	return mergePrimaryAgents([builtIn, user, project]);
}

export async function discoverPrimaryAgents(
	options: DiscoverPrimaryAgentsOptions,
