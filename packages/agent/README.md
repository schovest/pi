# @schovest/pi-agent-core

Stateful agent with tool execution and event streaming. Built on `@schovest/pi-ai`.

## Installation

```bash
npm install @schovest/pi-agent-core
```
...
```typescript
declare module "@schovest/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@schovest/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@schovest/pi-agent-core";
