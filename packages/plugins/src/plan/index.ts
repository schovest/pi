...
export default function planPlugin(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("plan-status", {
    ...
    handler: async (_args: string, ctx: ExtensionContext) => {
      ...
      const lines = state.currentPlan.steps.map(
        (s, i) =>
          `${i + 1}. [${s.status}] ${s.text}${s.result ? ` — ${s.result}` : ""}`,
      );
      ctx.ui.notify(
        `Plan: ${state.currentPlan.title} (${state.currentPlan.status})\n${lines.join("\n")}`,
        "info",
      );
    },
  });
  ...
  pi.registerCommand("plan-save", {
    ...
    handler: async (_args: string, ctx: ExtensionContext) => {
      ...
      ctx.ui.notify(
        `Plan "${state.currentPlan.title}" saved to .pi/plans/${state.currentPlan.id}.md`,
        "info",
      );
    },
  });
...
Do NOT attempt to make changes - just describe what you would do.`,
    ...
    if (state.currentPlan && state.currentPlan.status === "executing") {
      const remaining = state.currentPlan.steps.filter(
