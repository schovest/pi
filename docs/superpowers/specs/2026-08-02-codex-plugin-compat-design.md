...
日期：2026-08-02
状态：已批准
范围：packages/coding-agent + dist-assets
...
四层结构，对称于现有 `claude-plugin-manager.ts`：

1. **`core/codex-plugin-manager.ts`** — marketplace/manifest 解析、安装（git clone / 本地路径 / npm pack）、MCP→mcp.json 写入、skills 路径收集、settings 读写
2. **`core/codex-hooks.ts`** — hooks.json/manifest.hooks 解析 + 子进程执行器（stdin JSON / stdout JSON / exit 2=block / timeout / env 注入）
3. **内置扩展 `dist-assets/extensions/codex-hooks.ts`** — 把 codex hooks 注册为 Pi 扩展事件 handler；实时读取 `listConfiguredCodexPlugins()`，插件变更无需 reload
4. **CLI** — `package-manager-cli.ts` 新增 `codex-plugin` 子命令族
...
| --- | --- | --- |
| SessionStart | `session_start` + `context` | Pi reason（startup/new/resume/fork）映射为 codex source 值后做 matcher 匹配；additionalContext 暂存 Map，首次请求经 context 注入 developer 消息 |
| SessionEnd | `session_shutdown` | 通知类，忽略输出 |
| UserPromptSubmit | `input` | `decision: block` → `{action:"handled"}`；additionalContext 暂存注入 |
| PreToolUse | `tool_call` | deny → block+reason；`updatedInput` → 原地改写 input；additionalContext → 注入 |
...
| SubagentStart | `agent_start` | additionalContext 注入 |
| SubagentStop | `agent_end` | 通知类（block/continue 近似） |
| Stop | `turn_end` | `decision: block` → 暂存延续意图，下轮前经 `sendUserMessage` 触发 |
...
- 插件来源：marketplace 条目（local / git-subdir / npm）或直接安装源；npm 用 `npm pack`（不跑 lifecycle scripts）
- 安装后：写入 settings `codexPlugins[]`；MCP 经 mcp.json 注册（复用 `convertMcpServer` 模式，替换 `${PLUGIN_ROOT}`/`${CLAUDE_PLUGIN_ROOT}`）；skills 目录经 `PathMetadata`（`origin: "codex-plugin"`）合并进 resource-loader
- 旧格式 `commands` → 桥接扩展 `registerCommand` 注册斜杠命令 `/codex:<plugin>:<cmd>`

## 信任与安全

- 安装即信任（与 claude-plugin 一致）：`install` 时打印 hooks 摘要（事件 + command）供确认
- 子进程受 timeout 限制；exit≠0/2 且无有效 JSON 输出视为失败并告警
- `pi codex-plugin hooks list/disable <plugin>` 管理 hooks 开关（不做 hash 信任持久化）
...
pi codex-plugin hooks disable <plugin>
pi codex-plugin hooks enable <plugin>
```
...
1. 新/旧 marketplace.json 解析、新/旧 manifest 解析、hooks.json 解析（matcher/timeout/内联/数组）
2. hook 执行器：假脚本回显 JSON / exit 2 + stderr / 超时 → 断言 stdin 字段、env 替换、block 语义
3. 桥接映射：tool_call deny/rewrite、tool_result 替换、input handled、session_start→context 注入
4. CLI 命令解析（fake SettingsManager）

不跑真实 provider / 不触发 e2e。
